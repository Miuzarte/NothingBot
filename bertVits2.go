...
func (p *bertVitsPost) recordHist(groupId, userId int, userName, state string) {
	...
	text := strings.Replace(p.Text, "\n", "\\n", -1)
	content := ConvertToCSV(nowTime, groupId, userId, userName, state, p.Speaker, p.Lang, text)
	if err := appendToFile(
...
func checkBertVITS2(ctx *EasyBot.CQMessage) {
	//后端控制
	matches := ctx.RegFindAllStringSubmatch(regexp.MustCompile(`^(unload|refresh|exit|卸载|清理|退出).*(模型|model)$`))
	...
	matches = ctx.RegFindAllStringSubmatch(regexp.MustCompile(`(?s)让?岁己(用(中文|汉语|ZH|日文|日语|JP))?(说|复述)\s*(.*)`))
	if len(matches) > 0 {
		...
		text := trimOuterQuotes(unescape.Replace(matches[0][4]))
...
// 发送vits消息
func sendVitsMsg(ctx *EasyBot.CQMessage, text, lang string) error {
	log.Debug("[BertVITS2] Vits text: ", text)
	if len(strings.TrimSpace(text)) == 0 {
		return ctx.SendMsgReply("[BertVITS2] 文本输入不可为空！")
	}
	outputB64, err := bertVits2TTS(ctx, text, "", lang)
	if err != nil {
		log.Error("[BertVITS2] 发生错误: ", err)
		return ctx.SendMsgReply("[BertVITS2] 发生错误：", err.Error())
	}
	return ctx.SendMsg(bot.Utils.Format.VocalBase64(outputB64, true))
}
