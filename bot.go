...
func checkBotInternal(ctx *gocqMessage) {
	...
	if len(match) > 0 {
		...
		if len(call) > 0 { //记录喊话
			...
			ctx.sendForwardMsg(appendForwardNode(gocqForwardNodes{}, gocqNodeData{content: []string{
				"[NothinBot]" +
					"\n帮助信息" +
				...
				"注入消息：" +
					"\n“{@Bot}run{text}”" +
					"\n（“@Bot run[CQ:at,​qq=" + getSelfId().Str() + "]”）" +
					"\n输出相应消息，支持CQ码",
